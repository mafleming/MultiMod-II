...
\ for the bootloader and other features. These commands are specific to the
\ Winbond SPI flash device used by the MultiMod II. Not all spi flash devices
\ support per-block write protection. Specifically, the GigaDevice GD25Q16C
\ used by the Fomu device lacks a Status Register 3.
\
\ The fomu flash and similar devices can write-protect upper or lower segments
\ of flash in increments of 1/32 of total storage. For these devices, an
\ alternative approach to pretecting the bootloader is provided.

\ #############################################################
\ #######    SPI STATUS REGISTERS    ##########################
\ Note that not all devices have a third status register
...
: wrstatreg ( register-value status-register-num -- )
    ...
    \ Register number is $01 (1), $31 (2), or $11 (3)
...
\ #############################################################
\ #######    SPI BLOCK PROTECTION    ##########################
\ Note that not all devices have a third status register or
\ provide per-block protection. Check datasheet.
...
62 constant forthsize          \ Number of blocks in Forth directory
