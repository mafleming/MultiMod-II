...
Flash memory can't be written on an individual memory cell basis; a cell must be erased and then the contents of the cell written back. An entire sector must be erased before data can be written to cells in the sector. In the Winbond standard SPI flash device, sectors can be erased in 4 KB, 32 KB or 64 KB sized blocks.
...
- **spimode_std ( -- ) -** Set FPGA I/O to Standard mode.  
Commands and Data are sent in Standard transfer mode wherein **MISO** is an input to the FPGA and **MOSI**, **IO2**, and **IO3** are outputs.  
...
- **hp71b-1 -** Mark Top of Fixed Dictionary Words.  
