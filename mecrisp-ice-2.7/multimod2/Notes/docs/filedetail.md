...
- (Byte 0) **Entry Status.**
<br>
This byte indicates the entry status, whether it is **Empty**, **Valid**, or **Reclaimable**.
<br>
- (Byte 1) **Image Type.**
<br>
...
<br>
The image type is **IRAM**, **ROM**, **HARD ROM**, **TAKEOVER ROM**, and **DIRSIZE**. Note that the size of a HARD ROM and TAKEOVER ROM are fixed, so that the size bits are ignored.
<br>
The first entry of a directory is of type **DIRSIZE**. The lower four bits are ignored, and the Image Location field indicates the number of image blocks that are inclusive to the directory.
- (Bytes 2~3) **Image Location.**
<br>
...
- (Bytes 4~15) **Name.**
<br>
An image name can be up to 11 ASCII characters, padded with $FF bytes when less than 12 characters. The first byte indicates the number of characters in the name.
