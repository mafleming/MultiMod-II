...
uses crt, sysutils, zahlenzauber;
...
var pc : word = 0;
   ...
   rdepth : word = 0; // Not available in J1a, but useful for debugging
    ...
    lesezugriff, schreibzugriff, fetchzugriff : array[0..$1FFF] of longint;
...
begin
  ...
  if (pc and $2000) <> 0 then begin inc(lesezugriff[pc and $1FFF]); dpush(insn); pc := rpop shr 1; end // Memory fetch
  else
  begin
    inc(fetchzugriff[pc and $1FFF]);
        ...
        $6000: begin                                                                     // ALU
                ...
                rtos := rstack[0];
                 ...
                 case insn and $1F00 of
                   ...
                   $1800: tosN := dstack[2];
                   $1900: begin tosN := memory[tos shr 1]; inc(lesezugriff[(tos shr 1) and $1FFF]); end;
                 ...
                 if (insn and $0070) = $0030 then begin inc(schreibzugriff[(tos shr 1) and $1FFF]); memory[(tos shr 1) and $1FFF] := nos; end;  // Memory write
...
begin
  ...
  textcolor(green);   write('Gelesen  ');
  textcolor(cyan);    write('Geschrieben ');
  ...
  write('                     ');
  for adresse := 0 to $F do write(word2hex(2*adresse), ' ');

  for adresse := anfangsadresse to endadresse do
begin
     // if ((addresse shl 1) and $F = 0 then


    //if (lesezugriff[adresse] <> 0) or (schreibzugriff[adresse] <> 0) or (fetchzugriff[adresse] <> 0) then // or (memory[adresse] <> $0000) then
    begin
      ...
      if (adresse <> (letzte_adresse + 1)) or
         ...
         begin
           ...
           asciizeile := '';
           writeln;
           write('Adresse: ', word2hex((adresse and $FFF0) shl 1), ' Daten: ');
           if (adresse and $F) <> 0 then
           begin // Einrücken, falls es nicht direkt vorne beginnt.
             for einrueck := 1 to 5*(adresse and $F) do write(' ');
           end;
         end;
      // Farbe des Textes setzen !
//      if memory[adresse].belegt then textbackground(blue)
//                                  else textbackground(0);

      if fetchzugriff[adresse] <> 0
        then textcolor(red)
        else if schreibzugriff[adresse] = 1
             then textcolor(cyan)
             else if schreibzugriff[adresse] > 1
                  then textcolor(blue)
                  else if lesezugriff[adresse] > 0
                       then textcolor(green)
                       else begin textbackground(blue); textcolor(black); end;


     //   if lesezugriff[adresse] <> 0
     //          then textcolor(green{magenta})
     //          else if schreibzugriff[adresse] = 0
     //               then begin textbackground(blue); textcolor(black); end
     //               else if schreibzugriff[adresse] = 1
     //                    then textcolor(cyan)
     //                    else textcolor(blue);

       //        if (memory[adresse].lesezugriff or memory[adresse].schreibzugriff)
       //             then //textcolor(cyan)
       //                  begin
       //                    if memory[adresse].geschrieben <= 1 then textcolor(cyan)
       //                                                          else textcolor(blue);
       //                  end
      ...
      //if (schreibzugriff[adresse] > 1) then textbackground(brown);

      //write('(');
      //if memory[adresse].lesezugriff    then write('l') else write(' ');
      //if memory[adresse].schreibzugriff then write('s') else write(' ');
      //if memory[adresse].opcode         then write('b') else write(' ');
      //if memory[adresse].operand        then write('o') else write(' ');
      //if memory[adresse].calleinsprung  then write('c') else write(' ');
      //write(')');

      write(word2hex(memory[adresse]), ' ');

      if lesezugriff[adresse] > 0 then
      begin
        zeichen := memory[adresse] and $FF;
        if (zeichen >= 32) and (zeichen < 127) then asciizeile := asciizeile + chr(zeichen) else asciizeile := asciizeile + ' ';
        zeichen := memory[adresse] shr 8 and $FF;
        if (zeichen >= 32) and (zeichen < 127) then asciizeile := asciizeile + chr(zeichen) else asciizeile := asciizeile + ' ';
      end else asciizeile := asciizeile + '  ';

      textbackground(0);
      textcolor(0);

      letzte_adresse := adresse;
      //writeln(word2hex(adresse), ' ', byte2hex(memory[adresse].inhalt));
    end;
  end;
...
function dictionarynext(link : word) : word;
begin
  if link = 0 then dictionarynext := 0 else dictionarynext := (memory[link shr 1] shr 2) and $3FFE;
end;

procedure words(start : word);
var alterlink, link : word;
    namenslaenge : byte;
    i : word;

    codestart, codeend : word;
    unknown : word;
...
begin
  ...
  begin
    ...
    namenslaenge := memory[(link + 2) shr 1] and $7F;

    codestart := (link + 3 + namenslaenge + 1) and $FFFE;
    codeend   := alterlink - 2;

    write('Code: ', word2hex(codestart));
    write(' - ', word2hex(codeend));

    unknown := 0;
    for i := (codestart shr 1) to (codeend shr 1) do
    begin
    // write('*');
      if (fetchzugriff[i] = 0) and (lesezugriff[i] = 0) (* and (schreibzugriff[i] = 0) *) then inc(unknown);
    end;
    if unknown <> 0 then write(' Unknown: ', word2hex(unknown), ' ') else write(' Unknown: ---- ');

    write(' Name: ');

    for i := 0 to namenslaenge - 1 do
    begin
      if (link + 3 + i) and 1 = 0 then write(chr(memory[(link + 3 + i) shr 1] and $FF))
                                  else write(chr(memory[(link + 3 + i) shr 1] shr 8 and $FF))
    end;
    writeln;
    alterlink := link;
    link := dictionarynext(link);
  end;
end;
...
begin
  ...
  begin
    ...
    writeln('With code coverage checks included for testing');
    writeln('Usage:');
    writeln('  icecreammachine image dictionary-start-addr');
    writeln('  icecreammachine image dictionary-start-addr source');
    halt;
  end;
  ...
  fillchar(   lesezugriff, sizeof(   lesezugriff), 0);
  fillchar(schreibzugriff, sizeof(schreibzugriff), 0);
  fillchar(  fetchzugriff, sizeof(  fetchzugriff), 0);
  ...
  // speicherkarte(0, $1FFF); // Print memory map

  words(strtoint(paramstr(2)));
