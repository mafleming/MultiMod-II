...
: tw!      ( w t-addr -- )  tflash + l! ;
: tw@      ( t-addr -- w )  tflash + ul@ ;  \ Unsigned, as expected for 32 bit target memory contents.
...
: tail-call-optimisation ( opcode-addr -- ? )
    ...
    dup tw@ $e0000000 and $40000000 =
    if
        dup tw@ tmask 3 rshift and over tw! \ Transform call opcode into jmp opcode.
        true
    else
    \ No, this wasn't a call opcode. Maybe an ALU opcode without return stack usage which we can add the necessary bits for ret to ?
        dup tw@ $e000000c and $60000000 = if
...
:: @i ( addr -- x ) \ Effect similar to @ on final execution ( -- ) on compilation.
    ...
    r@ tw@ $7FFFFFFF and $40000000 + tcell / $40000000 or r> tw!   \ and replace that with a high-call opcode.

    \ Literal + Execute Sequence instead:
    \ tdp @ tcell - >r                       \ Get last opcode, which should be a literal
    \ r@ tw@ $40000000 or r> tw!              \ and prepare the address for a high-call.
    \ $00000010 2 rshift $40000000 or tw,      \ Insert a call to execute, which is close to the beginning of the nucleus. We cannot tick target definitions at this stage.
;

:: !i ( addr -- ) \ Effect similar to ! on final execution ( -- ) on compilation.
    \ High-Call opcode to store variable:
    tdp @ tcell - >r                                              \ Get last opcode, which should be a literal
    r@ tw@ $7FFFFFFF and $20000000 + tcell / $40000000 or r> tw!   \ and replace that with a high-call opcode.
;
