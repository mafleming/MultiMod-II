...
: literal ( w -- )
    dup $8000000000000000 and if
        invert recurse
        ~T alu
    else
        $8000000000000000 or tw,
    then
;
...
: tail-call-optimisation ( opcode-addr -- ? )
    ...
    dup tw@ $e000000000000000 and $4000000000000000 =
    if
    ...
    \ No, this wasn't a call opcode. Maybe an ALU opcode without return stack usage which we can add the necessary bits for ret to ?
        dup tw@ $e00000000000000c and $6000000000000000 = if
...
:: @i ( addr -- x ) \ Effect similar to @ on final execution ( -- ) on compilation.
    ...
    r@ tw@ $7FFFFFFFFFFFFFFF and $4000000000000000 + tcell / $4000000000000000 or r> tw!   \ and replace that with a high-call opcode.

    \ Literal + Execute Sequence instead:
    \ tdp @ tcell - >r                       \ Get last opcode, which should be a literal
    \ r@ tw@ $4000000000000000 or r> tw!              \ and prepare the address for a high-call.
    \ $0000000000000010 3 rshift $4000000000000000 or tw,      \ Insert a call to execute, which is close to the beginning of the nucleus. We cannot tick target definitions at this stage.
;
