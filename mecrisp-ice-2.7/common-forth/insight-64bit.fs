...
: name. ( Address -- )  ( If the address is Code-Start of a dictionary word, it gets named. )
  ...
  dup ['] s, 96 + = \ Is this a string literal ?
  if
    ."   --> s" [char] " emit space
    disasm-$ @ count type
    [char] " emit
  ...
  dup $4000000000000000 and if ."   --> " $3FFFFFFFFFFFFFFF and .x ." @" exit then
...
: alu. ( Opcode -- ) ( If this opcode is from an one-opcode definition, it gets named. This way inlined ALUs get a proper description. )
  ...
  dup $6000000000000127 = if ." >r"    drop exit then
  dup $6000000000000B11 = if ." r@"    drop exit then
  dup $6000000000000B1D = if ." r>"    drop exit then
  dup $600000000000000C = if ." rdrop" drop exit then

  $FFFFFFFFFFFFFF73 and
  >r
  forth @
  begin
    dup
  while
    dup cell+ count 127 and + aligned @ ( Dictionary First-Opcode )
        dup $E000000000000080 and $6000000000000080 =
        if
          $FFFFFFFFFFFFFF73 and r@ = if rdrop cell+ count 127 and type space exit then
        else
          drop
        then
...
: disasm-step ( -- )
  ...
  dup $8000000000000000 and                     if ." Imm  " $7FFFFFFFFFFFFFFF and       dup .x 6 spaces                      .x       exit then ( Immediate )
  dup $E000000000000000 and $0000000000000000 = if ." Jmp  " $1FFFFFFFFFFFFFFF and cells dup                                  .x name. exit then ( Branch )
  dup $E000000000000000 and $2000000000000000 = if ." JZ   " $1FFFFFFFFFFFFFFF and cells disasm-cont @ over max disasm-cont ! .x       exit then ( 0-Branch )
  dup $E000000000000000 and $4000000000000000 = if ." Call " $1FFFFFFFFFFFFFFF and cells dup                                  .x name. exit then ( Call )
                                                   ." Alu"   25 spaces dup alu. $80 and if ." exit" then                                         ( ALU )
;
...
: seec ( -- ) ( Continues to see )
  ...
  begin
    disasm-$ @ @
    dup  $E000000000000080 and $6000000000000080 =   ( Loop terminates with ret )
    swap $E000000000000000 and 0= or                 ( or when an unconditional jump is reached. )
