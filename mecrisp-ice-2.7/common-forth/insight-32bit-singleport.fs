...
: name. ( Address -- )  ( If the address is Code-Start of a dictionary word, it gets named. )
  ...
  dup ['] s, 48 + = \ Is this a string literal ?
  if
    ."   --> s" [char] " emit space
    disasm-$ @ count type
    [char] " emit
  ...
  dup $40000000 and if ."   --> " $1FFFFFFF and .x ." @" exit then
  dup $20000000 and if ."   --> " $1FFFFFFF and .x ." !" exit then
...
: alu. ( Opcode -- ) ( If this opcode is from an one-opcode definition, it gets named. This way inlined ALUs get a proper description. )
  ...
  dup $60000127 = if ." >r"    drop exit then
  dup $60000B11 = if ." r@"    drop exit then
  dup $60000B1D = if ." r>"    drop exit then
  dup $6000000C = if ." rdrop" drop exit then
  ...
  while
    dup cell+ count 127 and + aligned @ ( Dictionary First-Opcode )
        dup $E0000080 and $60000080 =
        if
          $FFFFFF73 and r@ = if rdrop cell+ count 127 and type space exit then
        else
          drop
        then
...
: disasm-step ( -- )
  ...
  dup $80000000 and             if ." Imm  " $7FFFFFFF and       dup .x 6 spaces                      .x       exit then ( Immediate )
  dup $E0000000 and $00000000 = if ." Jmp  " $1FFFFFFF and cells dup                                  .x name. exit then ( Branch )
  dup $E0000000 and $20000000 = if ." JZ   " $1FFFFFFF and cells disasm-cont @ over max disasm-cont ! .x       exit then ( 0-Branch )
  dup $E0000000 and $40000000 = if ." Call " $1FFFFFFF and cells dup                                  .x name. exit then ( Call )
                           ." Alu"   17 spaces dup alu. $80 and if ." exit" then                                         ( ALU )
;
...
: seec ( -- ) ( Continues to see )
  ...
  begin
    disasm-$ @ @
    dup  $E0000080 and $60000080 =   ( Loop terminates with ret )
    swap $E0000000 and 0= or         ( or when an unconditional jump is reached. )
