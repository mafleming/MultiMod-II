...
: number ( addr len -- 0 )
  ...
  fractional @ if swap then
  sign @ if dnegate then
  doubleresult @ if d# 2 else drop d# 1 then
    ...
    while
      ...
      if ( xt imm-flag ) \ Found.
        ...
        if    \ Compile mode
          ...
          dup foldability @ 1- u< invert \ Check if there are enough constants left
          foldability @ 0<> \ Check if the current definition is foldable
          and
          if \ Fold it !
            drop
            r> rdrop execute
