...
: sfind
    ...
    while
        ...
        if
            ...
            dup @ d# 7 and foldability !i               \ Three LSBs of Link are flags for foldability
  ...
  if
    ...
    if ( call-target )
      ...
      if
         ...
         h# A0000000 prev@xor \ Add $20000000 to address and remove MSB which denotes literal opcodes
         2/ 2/
         h# 40000000 or       \ Make call opcode
         prev !
         jmp tdrop
...
header-imm ;
:noname
    lastword @i ?dup if forth !i then
    texit
    t[
;
...
header decimal : decimal ( -- ) d# 10 base !i ;
header binary  : binary  ( -- ) d# 2  base !i ;
header hex     : hex     ( -- ) d# 16 base !i ;
...
: interpret
    ...
    while
      ...
      if depth d# 2 - constantfoldingpointer !i then \ Do not include the values of parse-name
